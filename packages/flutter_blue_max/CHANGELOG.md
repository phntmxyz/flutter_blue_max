...
## 1.34.5
* **[Feature]** add `FlutterBlueMax.logs` to get access to FBP logs
...
## 1.20.7
* **[Fix]**  events API was not accessible through `FlutterBlueMax.events`

## 1.20.6
* **[Add]** `FlutterBlueMax.events.mtu`

## 1.20.5
* **[Add]** `FlutterBlueMax.events.onNameChanged`
* **[Add]** `FlutterBlueMax.events.onServicesChanged`

## 1.20.4
* **[Rename]** `FlutterBlueMax.connectionEvents` -> `FlutterBlueMax.events.connectionState`
* **[Add]** `FlutterBlueMax.events.onCharacteristicReceived`
* **[Add]** `FlutterBlueMax.events.onDescriptorRead`
* **[Add]** `FlutterBlueMax.events.bondState`

## 1.20.3
* **[Add]** `FlutterBlueMax.connectionEvents`, a stream of all connection & disconnected events
* **[Add]** `FlutterBlueMax.connectedDevices`, to get currently connected devices
...
## 1.20.1
* **[Remove]** `FlutterBlueMax.connectedDevices`. This API needs more thought.
...
## 1.19.1
* **[Add]** new method `FlutterBlueMax.connectedDevices`
...
* **[Java]** rename `com.boskokg.flutter_blue_max` -> `com.lib.flutter_blue_max` to be more generic
...
## 1.15.1
* **[Fix]** `FlutterBlueMax.scanResults` should always return list copy to avoid iteration exceptions (regression in 1.15.0)
...
* **[Simplify]** removed `FlutterBlueMax.scan`. Use `FlutterBlueMax.scartScan(oneByOne: true)` instead.
...
## 1.14.1
* **[Improve]** Dart: each `FlutterBlueMaxException` should have unique code for handling
...
## 1.12.13
* **[Fix]** iOS: `FlutterBlueMax.isAvailable` 'int' is not a subtype of type 'FutureOr<bool>' (regressed in 1.12.10)
...
* **[Fix]** iOS: `FlutterBlueMax.isOn` returns 'no' first time even though it is on (bug in original `flutter_blue`)
...
* **[BREAKING CHANGE]** Dart: use static functions for `FlutterBlueMax` instead of `FlutterBlueMax.instance`. Multiple instances is not supported by any platform.
...
simplicity of the FlutterBlueMax codebase. Please submit bug reports.

* **[Breaking Change/Fix]** Android: When `read` is called `onValueChangedStream` is pushed to as well. This change was made to make both platforms behave the same way. It is an unavoidable limitation of iOS. See: https://github.com/boskokg/flutter_blue_max/issues/419
...
* **[Fix]** Dart: `FlutterBlueMax.state` variable not working (reggression in 1.8.6)
...
* **[rename]** Dart: `FlutterBlueMax.name` -> `adapterName`
* **[rename]** Dart: `BluetoothDevice.name` -> `localName`
* **[rename]** Dart: `FlutterBlueMax.state` -> `adapterState` 
