...
Breaking changes in FlutterBlueMax, listed version by version.

## 1.8.6
* **renamed:** `BluetoothDevice.id` -> `remoteId`
* **renamed:** `FlutterBlueMax.name` -> `adapterName`
* **renamed:** `BluetoothDevice.name` -> `platformName`
* **renamed:** `FlutterBlueMax.state` -> `adapterState`
...
i.e. `FlutterBlueMax.instance.startScan` becomes `FlutterBlueMax.startScan`
...
### `FlutterBlueMax.scan` was removed

**Option 1:** migrate to `FlutterBlueMax.startScan` with `oneByOne` parameter

**Option 2:** use the following extension (below)

```
extension Scan on FlutterBlueMax {
  static Stream<ScanResult> scan({
    List<Guid> withServices = const [],
    Duration? timeout,
    bool androidUsesFineLocation = false,
  }) {
    if (FlutterBlueMax.isScanningNow) {
        throw Exception("Another scan is already in progress");
    }

    final controller = StreamController<ScanResult>();

    var subscription = FlutterBlueMax.scanResults.listen(
      (r){if(r.isNotEmpty){controller.add(r.first);}},
      onError: (e, stackTrace) => controller.addError(e, stackTrace),
    );

    Future scanComplete = FlutterBlueMax.isScanning.skip(1).where((e) => e == false).first;

    FlutterBlueMax.startScan(
...
### `FlutterBlueMax.startScan` doesn't return List<ScanResult> anymore

**Option 1:** migrate to `FlutterBlueMax.scanResults`. Example code:

```
Stream<BluetoothDevice?> myDeviceStream = FlutterBlueMax.scanResults
    .map((list) => list.first)
...
await FlutterBlueMax.startScan(timeout: Duration(seconds: 10), oneByOne:true);
...
extension Scan on FlutterBlueMax {
  static Future<List<ScanResult>> startScanWithResult({
    ...
    var subscription = FlutterBlueMax.scanResults.listen((result) {
      output = result;
    ...
    // wait scan complete
    await FlutterBlueMax.isScanning.where((e) => e == false).first;
...
### `await FlutterBlueMax.startScan()` does not wait for scan completion anymore

Use `isScanning` to detect completion instead.

```
await FlutterBlueMax.startScan(timeout: Duration(seconds:15));
await FlutterBlueMax.isScanning.where((value) => value == false).first;
```
