...
FlutterBlueMax supports nearly every feature on all supported platforms: iOS, macOS, Android, Linux, Web.

## No Dependencies

FlutterBlueMax has zero dependencies besides Flutter, Android, iOS, and macOS themselves.
...
**Streams:** Streams returned by FlutterBlueMax never emit any errors and never close. There's no need to handle `onError` or `onDone` for  `stream.listen(...)`. The one exception is `FlutterBlueMax.scanResults`, which you should handle `onError`.
...
FlutterBlueMax.setLogLevel(LogLevel.verbose, color:false);

// optional
FlutterBlueMax.logs.listen((String s){
...
// Note: The platform is initialized on the first call to any FlutterBlueMax method.
if (await FlutterBlueMax.isSupported == false) {
...
var subscription = FlutterBlueMax.adapterState.listen((BluetoothAdapterState state) {
    print(state);
...
var subscription = FlutterBlueMax.onScanResults.listen((results) {
...
FlutterBlueMax.cancelWhenScanComplete(subscription);

// Wait for Bluetooth enabled & permission granted
// In your real app you should use `FlutterBlueMax.adapterState.listen` to handle all states
await FlutterBlueMax.adapterState.where((val) => val == BluetoothAdapterState.on).first;

// Start scanning w/ timeout
// Optional: use `stopScan()` as an alternative to timeout
await FlutterBlueMax.startScan(
  withServices:[Guid("180D")], // match any of the specified services
  withNames:["Bluno"], // *or* any of the specified names
  timeout: Duration(seconds:15));

// wait for scanning to stop
await FlutterBlueMax.isScanning.where((val) => val == false).first;
```
...
FlutterBlueMax automatically listens to the Services Changed Characteristic (0x2A05)

In FlutterBlueMax, we call it `onServicesReset` because you must re-discover services.
...
```dart
List<BluetoothDevice> devs = FlutterBlueMax.connectedDevices;
for (var d in devs) {
    print(d);
}
```
...
List<BluetoothDevice> devs = await FlutterBlueMax.systemDevices(withServices);
for (var d in devs) {
...
FlutterBlueMax.events.onConnectionStateChanged.listen((event)) {
...
### Add the `flutter_blue_max` plugin

We recommend that you pin to a specific version of the `flutter_blue_max` plugin for maximum stability and to avoid small breaking changes.

```shell
flutter pub add flutter_blue_max:x.y.z
```

`flutter_blue_max` is a [federated plugin](https://docs.flutter.dev/packages-and-plugins/developing-packages#federated-plugins) with endorsed platform implementations therefore you only need to add the "app-facing package" to your `pubspec.yaml` file.

### Change the minSdkVersion for Android

flutter_blue_max is compatible only from version 21 of Android SDK so you should change this in **android/app/build.gradle**:
...
```
-keep class com.lib.flutter_blue_max.* { *; }
```
...
**This is an advanced use case**. FlutterBlueMax does not support everything. You may have to fork it. PRs are welcome.
...
```
FlutterBlueMax.setOptions(restoreState: true);
```
...
The easiest way to debug issues in FlutterBlueMax is to make your own local copy.
...
```
  flutter_blue_max:
    path: /user/downloads/flutter_blue_max
```

Now you can edit the FlutterBlueMax code yourself.
...
You can also use `FlutterBlueMax.adapterState.listen(...)`. See [Usage](#usage).
...
```
if (await FlutterBlueMax.adapterState.first == BluetoothAdapterState.unknown) {
...
You are forgetting to cancel the original `FlutterBlueMax.adapterState.listen` resulting in multiple listeners.

```dart
// tip: using ??= makes it easy to only make new listener when currently null
final subscription ??= FlutterBlueMax.adapterState.listen((value) {
    // ...
});
...
List<BluetoothDevice> system = await FlutterBlueMax.systemDevices;
for (var d in system) {
...
```dart
// tip: using ??= makes it easy to only make new listener when currently null
final subscription ??= FlutterBlueMax.device.connectionState.listen((value) {
    // ...
});
...
It means your device stopped working. FlutterBlueMax cannot fix it.
...
FlutterBlueMax cannot fix these errors. You are doing something wrong & your device is responding with an error.
...
If you just added flutter_blue_max to your pubspec.yaml, a hot reload / hot restart is not enough.
