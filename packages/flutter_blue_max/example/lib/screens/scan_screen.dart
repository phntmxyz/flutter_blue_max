...
class _ScanScreenState extends State<ScanScreen> {
  ...
  void initState() {
    ...
    _scanResultsSubscription = FlutterBlueMax.scanResults.listen((results) {
      if (mounted) {
    ...
    _isScanningSubscription = FlutterBlueMax.isScanning.listen((state) {
      if (mounted) {
  ...
  Future onScanPressed() async {
    try {
      ...
      _systemDevices = await FlutterBlueMax.systemDevices(withServices);
    } catch (e, backtrace) {
  ...
  Future onRefresh() {
    if (_isScanning == false) {
      FlutterBlueMax.startScan(timeout: const Duration(seconds: 15));
    }
    if (mounted) {
      setState(() {});
    }
